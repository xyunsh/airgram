...
function unwrap<ContextT>(handler: Middleware<ContextT>): MiddlewareFn<ContextT> {
...
function safePassThru<ContextT>(): MiddlewareFn<ContextT> {
    return (_ctx: ContextT, next: NextFn | unknown): Promise<unknown> =>
        typeof next === 'function' ? next() : Promise.resolve();
}

function lazy<ContextT>(factoryFn: (ctx: ContextT) => Promise<Middleware<ContextT>>): MiddlewareFn<ContextT> {
    return (ctx: ContextT, next?: NextFn): Promise<Middleware<ContextT>> =>
        Promise.resolve(factoryFn(ctx)).then(
            (middleware: Middleware<ContextT>): Middleware<ContextT> => unwrap<ContextT>(middleware)(ctx, next || noop),
        );
}
...
function branch<ContextT>(
    ...
    return lazy(
        (ctx): Promise<Middleware<ContextT>> =>
            Promise.resolve(predicate(ctx)).then(
                (value): Middleware<ContextT> => (value ? trueMiddleware : falseMiddleware),
            ),
    );
}
...
class Composer<ContextT extends BaseTdObject> implements BaseComposer<ContextT> {
    ...
    public on(predicateTypes: string | string[], ...fns: Middleware<ContextT>[]): void {
