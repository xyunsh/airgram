...
function createState(starting: Record<string, unknown>): ContextState {
    ...
    const setState: SetStateFn = (
        next: Record<string, unknown> | ((state: Record<string, unknown>) => Record<string, unknown>),
    ) => {
...
export class AirgramCore<ProviderT extends TdProvider> implements Instance<ProviderT> {
    ...
    public readonly on: MiddlewareOn = (predicateTypes: string | string[], ...fns: Middleware<any>[]): void => {
    ...
    public use(...fns: Middleware<Context>[]): void {
    ...
    private callApi<ParamsT extends TdObject | undefined, ResultT extends TdObject>(
        ...
        return this.createContext<ApiResponse<ParamsT, ResultT>>(request.method, {
            options: options || {},
            request,
        }).then(
            (ctx) =>
    ...
    private async createContext<T>(_: string, props: Record<string, any>): Promise<T> {
        ...
        const defineProperties = (obj: Record<string, any>) =>
    ...
    private handleUpdate(update: BaseTdObject, state: Record<string, any> = {}): Promise<unknown> {
        return this.createContext<UpdateContext<TdObject>>(update._, {
            update,
            options: { state },
