...
export class TdJsonClient extends BaseJsonClient {
    ...
    public constructor({ command, models, timeout }: TdJsonConfig) {
        ...
        this.client = ffi.Library(command ? resolvePath(command) : resolvePath(DEFAULT_COMMAND), {
    ...
    private async handleResponse(clientId: number): Promise<void> {
        ...
        if (requestId) {
            ...
            if (resolve) {
                ...
                this.handleError(
                    new Error(
                        `[TdProxy] request handler for the client ${clientId} not found. Missed update: ${JSON.stringify(
                            response,
                        )}`,
                    ),
                );
