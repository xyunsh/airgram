...
export const createProviderFactory: CreateProviderFactoryFn<Provider, BaseJsonClient> = (
    tdJsonClient: BaseJsonClient,
) => {
...
export class Airgram extends AirgramCore<Provider> {
    ...
    public constructor(configOrClient: ExtendedConfig | BaseJsonClient, config?: Config) {
        if (configOrClient instanceof BaseJsonClient && config) {
            ...
            if (models) {
                throw new Error(
                    'For using models you need to create TdJsonClient manually. See: https://airgram.netlify.app/api-reference/tdjsonclient',
                );
            }
